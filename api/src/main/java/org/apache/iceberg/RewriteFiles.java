...
public interface RewriteFiles extends SnapshotUpdate<RewriteFiles> {
   ...
   * @param filesToAdd     files that will be added, cannot be null or empty.
   * @param sequenceNumber sequence number to use for all data files added
   * @param needVerification   need verification : true or false; Flink writer verfication can be false
   * @return this for method chaining
   */
  RewriteFiles rewriteFiles(Set<DataFile> filesToDelete, Set<DataFile> filesToAdd, long sequenceNumber, boolean needVerification);
