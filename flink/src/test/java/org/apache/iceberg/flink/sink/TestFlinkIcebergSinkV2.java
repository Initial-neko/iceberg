...
public class TestFlinkIcebergSinkV2 extends TableTestBase {
  ...
  public void testChangeLogOnIdKey() throws Exception {
    ...
    testChangeLogs(ImmutableList.of("id"), row -> row.getField(ROW_ID_POS), elementsPerCheckpoint, false, expectedRecords);
  }
  ...
  public void testChangeLogOnDataKey() throws Exception {
    ...
    testChangeLogs(ImmutableList.of("data"), row -> row.getField(ROW_DATA_POS), elementsPerCheckpoint, false, expectedRecords);
  }
  ...
  public void testChangeLogOnIdDataKey() throws Exception {
    ...
    testChangeLogs(ImmutableList.of("data", "id"), row -> Row.of(row.getField(ROW_ID_POS), row.getField(ROW_DATA_POS)),
        elementsPerCheckpoint, false, expectedRecords);
  }
  ...
  public void testUpsertOnIdKey() throws Exception {
    ...
    if (!partitioned) {
      testChangeLogs(ImmutableList.of("id"), row -> row.getField(ROW_ID_POS),
              elementsPerCheckpoint, true,expectedRecords);
    } else {
      AssertHelpers.assertThrows("Should be error because equality field columns don't include all partition keys",
              ...
              () -> {
                testChangeLogs(ImmutableList.of("id"), row -> row.getField(ROW_ID_POS),  elementsPerCheckpoint,true,
                        expectedRecords);
                return null;
              });
    }
